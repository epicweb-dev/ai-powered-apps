...
export function ProductChat() {
	...
	// 🐨 Add `forwardedProps` to the options object below so the server route
	//    can tell which product the customer is looking at on every request.
	//    The panel also shows up on pages without a specific product (the
	//    products listing, the cart, the home page), so make it conditional:
						...
						{messages.map((m) => (
							...
							>
								{m.parts.map((part, i) => {
									...
									// 🐨 You turned on reasoning in api.chat.tsx, so the model now
									//    emits `thinking` parts alongside `text` parts. Uncomment
									//    Marty 💰's branch below to show them in a collapsible row.
									/*
