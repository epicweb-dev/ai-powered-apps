 ...
 *
 * The `sendHandRolled` function + the `reply`/`error`/`isLoading` state
 * below are a deliberately hand-rolled fetch wrapper. The next exercise
 * replaces this whole block with TanStack AI's `useChat`.
 */
export function ProductChat() {
	...
	// Hand-rolled chat state — replaced by `useChat` in the next exercise.
	const [reply, setReply] = useState('')
	const [error, setError] = useState<string | null>(null)
	const [isLoading, setIsLoading] = useState(false)

	// Hand-rolled chat submission — replaced by `useChat`'s
	// `sendMessage` in the next exercise.
			...
			{isOpen ? (
				<div className="flex h-[32rem] w-[22rem] flex-col overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-900">
					...
					<div className="flex-1 space-y-4 overflow-y-auto p-4">
						...
						{/* The reply preview + error bubble below belong to the
						    hand-rolled fetch. They'll be replaced by `useChat`'s
						    message list in the next exercise. */}
						{reply ? (
					...
					{/* The form's `onSubmit` calls the hand-rolled fetch. The next
					    exercise points it at `useChat`'s `sendMessage` instead. */}
					<form
